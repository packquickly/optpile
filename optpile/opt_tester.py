...
from jaxtyping import Array, Bool, Float, Int, PRNGKeyArray, Scalar, ScalarLike
from tqdm import tqdm

from .all_problems import all_problem_list
...
class OptTester(eqx.Module):
    ...
    def run(
        ...
        atol: float,
        rtol: float,
        termination_condition: Termination = Termination.CAUCHY,
        ...
        **Arguments:**
        - `termination_condition`: Which termination condition to use. A
        ...
        len(all_problem_list)
        for (i, problem) in tqdm(enumerate(all_problem_list)):
    ...
    def single_run(
        ...
        *,
        ...
        if problem.minimum.min is not None:
            successes = (
                jnp.abs(problem.minimum.min - min_found)
                / jnp.abs(atol + problem.minimum.min * rtol)
                < 1
            )
        else:
            successes = steps < soln.stats["max_steps"]
    ...
    def single_full_history_run(
        ...
        *,
        ...
        # if not termination_condition == Termination.MAX_STEPS:
        #     losses = jnp.where(index < steps, losses, losses[steps - 1])
...
def solve_with_history(
    ...
    def body_fn(carry):
        ...
        if termination_condition == Termination.MAX_STEPS:
